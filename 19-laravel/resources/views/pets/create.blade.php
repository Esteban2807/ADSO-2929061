    ...
    <div class="card text-white md:w-[720px] w-[320px] bg-black/20 p-4 mb-4 rounded">
        <form method="POST" action="{{ url('pets') }}" class="flex flex-col md:flex-row gap-4 mt-4" enctype="multipart/form-data">
            ...
            <div class="w-full md:w-[320px]">
                <div class="avatar flex flex-col gap-1 items-center justify-center cursor-pointer hover:scale-105 transition ease-in">
                    <div id="upload" class="mask mask-squircle w-48">
                        <img id="preview" src="{{ asset('images/no-photo.jpg') }}" class="object-cover" />
                    </div>
            ...
            <div class="w-full md:w-[320px]">
                ...
                <input type="number" step="0.1" class="input bg-[#0009] outline-0" name="weight" placeholder="10.5" value="{{ old('weight') }}">
                @error('weight')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
                {{-- Age --}}
                <label class="label text-white">Age (years):</label>
                <input type="number" class="input bg-[#0009] outline-0" name="age" placeholder="2" value="{{ old('age') }}">
                @error('age')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
            </div>
            <div class="w-full md:w-[320px]">
                ...
                <input type="text" class="input bg-[#0009] outline-0" name="breed" placeholder="Criollo" value="{{ old('breed') }}">
                @error('breed')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
                {{-- Location --}}
                ...
                <textarea class="textarea bg-[#0009] outline-0 h-24" name="description" placeholder="A friendly dog...">{{ old('description') }}</textarea>
                @error('description')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
